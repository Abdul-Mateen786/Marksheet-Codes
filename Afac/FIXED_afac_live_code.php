...
// if($class_rank > 80){
    if($stud_std == '1' || $stud_std == '2' || $stud_std == '3' || $stud_std == '4' || $stud_std == '5' || $stud_std == '6' || $stud_std == '7' || $stud_std == '8' || $stud_std == '9' || $stud_std == '10'){
   $subject_sequence =['english','hindi','marathi','maths','science_a','science_b','science','history','geography','social_science','art','w.ex','p_e','rsp/de','s_d'];
...
<div class="marksheet-div" style="padding:3%;">
    ...
    <div>
        ...
        <span style="font-size:1em;font-weight:300;"> </span><b>Date of Birth:</b> <?php echo $stud_dob;  ?>&emsp;
        <span style="font-size:1em;font-weight:300;"> </span><b>G. R. No:</b> <?php echo $gr_number;  ?><br>
    </div>
    <div class="grid-container">
        <div class="grid-1">
            <table style="width:100%;border-style:solid; border-collapse: collapse; line-height:20px;border:1px;font-size:0.8rem;" class= "cust_table">
                <tr>
                <?php
       ...
       //------------Merge Multiple Subjects (Combined Subjects)--------------------------------------------
            //Making SINGLE-SUBJECT-WISE EXAM (eg: EVS1+EVS2=EVS)
             $tobe_merged = ['history'=>'Social_Science','geography'=>'Social_Science','science_a'=>'Science','science_b'=>'Science'];// Alias : value to search in exams | Pretty_Name: Final(Merged Sub-Name)
             $merged_exams = [];
             $all_exams = [];
             foreach ($tobe_merged as $alias => $pretty_name) {
                 foreach ($sem1 as $key => $value) {
                    $subname = array_column($value['exam'], 'subject_name');
                     if (stripos(str_replace(" ","_",$subname[0]),$alias) !== FALSE) {
                         // $merged_exams[$pretty_name][] = $value;
                         foreach ($value['exam'] as $k1 => $v1) {
                             // $all_exams[$v1['exam_id']]=$v1['exam_id'];
                             $merged_exams[$pretty_name][$v1['exam_id']][] = $v1;
                         }
                         unset($sem1[$key]);
                     }
                 }
             }
             // echo print_array($merged_exams);die();
             $merge_new = [];
             foreach ($merged_exams as $key => $value) {
                 foreach ($value as $k1 => $v1) {
                     $marks_obt = array_sum(array_column($v1,'marks_obtained'));
                     $total_marks = array_sum(array_column($v1,'total_marks'));
                     $passing_marks = array_sum(array_column($v1,'passing_marks'));
                     $grace = array_sum(array_column($v1,'grace'));
                     $merge_new[$key]['exam'][$k1] = [
                         'exam_id'       => $v1[0]['exam_id'],
                         'exam_table_id' => $v1[0]['exam_table_id'],
                         'exam_name'     => $v1[0]['exam_name'],
                         'pes_id'        => $v1[0]['pes_id'],
                         'pcs_id'        => $v1[0]['pcs_id'],
                         'sem'           => $v1[0]['sem'],
                         'grace'         => $grace,
                         ...
                         'subject_name'  => ucwords(str_replace("_"," ",$key)),
                     ];
                 }
                 $sem1[]['exam'] = array_values($merge_new[$key]['exam']);
             }
             //---RESEQUENCE THE SUBJECTS SEQUENCE AS PER YOUR REQUIREMENT AND SUBJECT NAME----------
             //$subject_sequence =['english','marathi','maths', 'e.v.s','e.v.s 1','e.v.s 2','art_edu','work-exp','phy-edu','computer,'];
            // $subject_sequence =['english','marathi','hindi','maths','so.sci','history','geography','art_edu','work-exp','e.v.s_2','phy-edu','computer','art','craft','sports','p.e','drawing','p.edu'];
            //  $tmp_sem1 = $sem1; unset($sem1);
            //  foreach ($subject_sequence as $k1 => $v1) {
            //      foreach ($tmp_sem1 as $k2 => $v2) {
            //          if (stripos($v2['exam'][0]['subject_name'],$v1) !== FALSE) {
            //              $sem1[] = $v2;
            //          }                
            //      }                
            //  }
            //-----------------END MERGE COMBINED EXAMS------------------------------
            // echo print_array($sem1);die();
       ...
       //------------Merge Multiple Subjects (Combined Subjects)--------------------------------------------
            //Making SINGLE-SUBJECT-WISE EXAM (eg: EVS1+EVS2=EVS)
             ...
             foreach ($merged_exams as $key => $value) {
                 ...
                 $sem2[]['exam'] = array_values($merge_new[$key]['exam']);
             }
             //---RESEQUENCE THE SUBJECTS SEQUENCE AS PER YOUR REQUIREMENT AND SUBJECT NAME----------
             //$subject_sequence =['english','marathi','maths', 'e.v.s','e.v.s 1','e.v.s 2','art_edu','work-exp','phy-edu','computer,'];
            // $subject_sequence =['english','marathi','hindi','maths','so.sci','history','geography','art_edu','work-exp','e.v.s_2','phy-edu','computer','art','craft','sports','p.e','drawing','p.edu'];
            //  $tmp_sem2 = $sem2; unset($sem2);
            //  foreach ($subject_sequence as $k1 => $v1) {
            //      foreach ($tmp_sem2 as $k2 => $v2) {
            //          if (stripos($v2['exam'][0]['subject_name'],$v1) !== FALSE) {
            //              $sem2[] = $v2;
            //          }                
            //      }                
...
<div id="school_name" style="width:85%;">
    ...
    <span style="font-size:1em;font-weight:300;"> </span><b>G. R. No:</b> <?php echo $gr_number;  ?><br>
    <span style="font-size:1em;font-weight:300;"> </span><b>Date of Birth:</b> <?php echo $stud_dob;  ?>
    </div>
    <br>
...
<table style="width:100%;border-style:solid; border-collapse: collapse; line-height:20px;border:1px;font-size:0.8rem;">
    ...
    <tr>
      <td colspan="6" class="center"> <b><font color="black"> FIRST SEMESTER<font></b></td>
    </tr>
    ...
    if(count($sem1)){
            ...
            // echo print_array($v1);die();
             $sem1_table.="<tr class = 'marks_row'><td>".$v1['subject_name']."</td>";
                    ...
                    foreach ($v1['exam'] as $k2 => $v2) {
                        ...
                        $v2['marks_obtained']=isset($v2['marks_obtained']) ? $v2['marks_obtained'] : 0;
                        $v2['total_marks']=isset($v2['total_marks']) ? $v2['total_marks'] : 0;
                        $total_marks_obtained += $v2['marks_obtained'];//error was here
                        $total_max_marks += $v2['total_marks'];//error was here
                    }
     ...
     <!-- <table class="my-table" style="float:left; height:20%;margin-right: 2rem;"> -->
              <div style="margin-top:2%;">
                <div style="float:left;"><u>Signature:</u></div><br><br>
                <div style="float:left;">1)<u>Class Teacher:</u></div><br><br>
                <div style="float:left;">2)<u>Head Mistress:</u></div><br><br>
                <div style="float:left;">3)<u>Parent / Guardian:</u></div>
              </div>
            ...
            <table style= "float:right;border-collapse:collapse;margin-top:-4%; font-size:75%; line-height: 22px;">
...
function check_grade3($grade)
{
    $remarks = 'NA';
    // echo $grade;die('reached');
    ($grade == 'C-2') ? $remarks =  'Need more motivation and hardwork' : FALSE;
    ($grade == 'C-1') ? $remarks =  'Needs to increase efforts in the subject' : FALSE;
    ($grade == 'B-2') ? $remarks =  'Takes pride in work well done ' : FALSE;
    ($grade == 'B-1') ? $remarks =  'Has good subject knowledge' : FALSE;
    ($grade == 'A-2') ? $remarks =  'With the little efforts he can do better in the subject' : FALSE;
    ($grade == 'A-1') ? $remarks =  'Has shown great in the subject skills' : FALSE;
    return $remarks;
    // if($marks <= 20){ $remarks = '';}
    // elseif($marks >= 21 && $marks <= 32)    {$remarks =  '';}
    // elseif($marks >= 33 && $marks <= 40)    {$remarks =  'Not fit for the current standard';}
...
// end
    ...
    $sem1_table .= "<td class ='td_class center'>".$marks_obt_tot_sem1_sm."</td><td class ='td_class center'>". $grand_marks_obt_tot_sem1."</td><td class ='td_class center'>".check_grade2($percent)."</td></tr>";
...
$sem1_table .= "<td class ='td_class center'>".check_grade3(check_grade2($percent))."</td></tr>";
